...
class EventBus
{
    public function sendEvent(Event $event): void
    {
        ...
        $channel->exchange_declare('events', 'direct', false, true, false);
        $msg = new AMQPMessage((string)$event);
        $eventHandle = $event::getService() . "-" .  $event::getEventName() . "-" . $event::getEventVersion();
        $channel->basic_publish($msg, 'events', $eventHandle);
        $channel->close();
        $connection->close();
    }

    public function listenEvent(callable $callback, string $className): void
    {
        ...
        $channel->queue_bind($queue_name, 'events', $eventHandle);
